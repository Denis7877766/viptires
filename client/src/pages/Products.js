...
class Catalog extends React.Component {
    ...
    getData = async () => {
        try{    
            ...
            const param = this.props.match.params.brand
            ...
            if(param != null && param != undefined){
              const brand = param.toString().toLowerCase()
              console.log('CURRENT PAGE BRAND:', brand)
                // GET PRODUCTS
                await axios 
                .get('/api/products')
                .then(res => {
                    if(res.status === 200){ 
                        ...
                        if(brand.includes('secondary')){
                            filtered_data = res.data.filter(record => {
                                return  record.condition.toString().toLowerCase().includes('secondary')
                            })
                        }else{
                            filtered_data = res.data.filter(record => {
                                            return record.brand.toString().toLowerCase().includes(brand) && !record.condition.toString().toLowerCase().includes('secondary')
                            })     
                        }   
                        if(filtered_data.length){
                            console.log('Filtered:', filtered_data)
                            filtered_data = filtered_data.sort(compare).reverse()   
                            this.setState({products: filtered_data})
                        }else{ 
                          this.setState({noProductsErrMessage: true})
                        }

                        console.log('გაფილტრული: ', filtered_data)
                    }else{
        ...
        try{
            ...
            if(filterBy === 'category'){ 
                ...
                if(width_filter_selected === 'all' && radius_filter_selected === 'all' && height_filter_selected === 'all' && current_season === 'season'){
                    ...
                    if(products_all.length){
                        this.setState({products_filtered : products_all, noProductsErrMessage: false, apiError1: false})
                    }else{
                        this.setState({products_filtered : products_all, noProductsErrMessage: true, apiError1: false})
                     }
                    ...
                    if(filteredProducts.length){
                        this.setState({noProductsErrMessage: false, apiError1: false, products_filtered: filteredProducts}) 
                    }else{
                        this.setState({noProductsErrMessage: true,   products_filtered: []}) 
                    }
  ...
  render() {
    ...
    return (
      <div>
        ...
        {  this.state.products_searched === true && this.state.products_filtered.length < 1  && (
            <div style={{marginBottom:200}}>
            </div>
        )}  
              
        { this.state.apiError2 === false && this.state.products_searched === false &&   this.state.products.length > 0 && (
                ...
                <div  key={i} >
                    ...
                    {item.inSale === true ? (
                        ...
                        <div className="price"> 
                            ...
                            ) : item.noPrice === true? <span style={{fontSize: 10}}> {this.props.t("CatalogPage.noPrice")} </span>   :  this.state.currencyInUsd ? Math.ceil( Number( item.price) / Number(this.state.currency_rate)) :  item.price}
